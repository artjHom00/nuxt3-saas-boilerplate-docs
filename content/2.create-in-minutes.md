...
> ❗ Make sure to create a new product in [Lemon Squeezy dashboard](https://app.lemonsqueezy.com/dashboard) & replace the "lemonsqueezyId" with the actual product id. Then set up the [webhooks](/guide/payments#step-4-add-webhooks) so that new clients could register on the website once paid.
